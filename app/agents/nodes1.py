...
from app.db.database import get_db_connection  # Настоящий импорт БД
from app.agents.state1 import AgentState

# Модель и ключи берём из общих настроек проекта (читаются из .env через pydantic).
...
def sql_agent_node(state: AgentState) -> AgentState:
    ...
    try:
        ...
        state["raw_result"] = json.dumps(raw_data, ensure_ascii=False, default=str)
